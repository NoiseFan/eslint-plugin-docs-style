...
import type { OffsetRange } from '@/types/custom-container'
...
export default createRule<Options, MessageIds>({
  ...
  defaultOptions: [],
  create(context) {
    const ignoredRanges: OffsetRange[] = []
    ...
    return {
      ...
      'root:exit': function (node: Root) {
        const { position } = getNodePosition(node)
        /* v8 ignore if -- @preserve */
        if (!position)
          return

        const issues = getCustomContainerPaddingIssues(context.sourceCode.text, ignoredRanges)
