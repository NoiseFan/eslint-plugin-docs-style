...
import { isLinkNode } from '@/parser/ast'
...
describe('getParsedNodeContext', () => {
  ...
  it('throws when the requested node is missing', () => {
    expect(() => getParsedNodeContext('Plain text.', isLinkNode)).toThrow('Expected markdown fixture to contain the requested node.')
  })
})
...
describe('getParsedLinkContext', () => {
  ...
  it('throws when the fixture does not contain a link', () => {
    expect(() => getParsedLinkContext('Plain text.')).toThrow('Expected markdown fixture to contain a link.')
  })
})
